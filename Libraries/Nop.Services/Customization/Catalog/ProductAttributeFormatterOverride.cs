...
namespace Nop.Services.Catalog
{
    ...
    /// </summary>
    public partial class ProductAttributeFormatterOverride : ProductAttributeFormatter
    {
        #region Fields
        ...
        public ProductAttributeFormatterOverride(ICurrencyService currencyService,
            ...
            : base(currencyService, downloadService, localizationService, priceCalculationService, priceFormatter,
                productAttributeParser, taxService, webHelper, workContext, shoppingCartSettings)
            ...
            bool allowHyperlinks = true)
        {
                ...
                foreach (var attribute in _productAttributeParser.ParseProductAttributeMappings(attributesXml))
                {
                    ...
                    if (!attribute.ShouldHaveValues())
                    {
                        foreach (var value in _productAttributeParser.ParseValues(attributesXml, attribute.Id))
                        {
                            ...
                            {
                                //multiline textbox
                                var attributeName = _localizationService.GetLocalized(attribute.ProductAttribute,
                            ...
                            else if (attribute.AttributeControlType == AttributeControlType.FileUpload)
                            {
                                ...
                                if (download != null)
                                {
                                    ...
                                    var attributeText = allowHyperlinks
                                        ? $"<a href=\"{_webHelper.GetStoreLocation(false)}download/getfileupload/?downloadId={download.DownloadGuid}\" class=\"fileuploadattribute\">{fileName}</a>"
                                        : fileName;
                            ...
                            else
                            {
                                ...
                                formattedAttribute =
                                    $"{_localizationService.GetLocalized(attribute.ProductAttribute, a => a.Name, _workContext.WorkingLanguage.Id)}: {value}";
                    ...
                    {
                        foreach (var attributeValue in _productAttributeParser.ParseProductAttributeValues(
                            attributesXml, attribute.Id))
                        {
                            var formattedAttribute =
                                $"{_localizationService.GetLocalized(attribute.ProductAttribute, a => a.Name, _workContext.WorkingLanguage.Id)}: {_localizationService.GetLocalized(attributeValue, a => a.Name, _workContext.WorkingLanguage.Id)}";
                            ...
                            if (renderPrices)
                            {
                                ...
                                else
                                {
                                    ...
                                    var priceAdjustmentBase = _taxService.GetProductPrice(product,
                                        attributeValuePriceAdjustment, 1, customer, out var _, out var _);
                                    var priceAdjustment =
                                        _currencyService.ConvertFromPrimaryStoreCurrency(priceAdjustmentBase,
                                            _workContext.WorkingCurrency);

                                    if (TaxSplittingHelper.IsProductTaxSplittingOverridenByAttributeValue(
                                ...
                                attributeValue.AttributeValueType == AttributeValueType.AssociatedToProduct)
                            {
                                ...
                                if (attributeValue.Quantity > 1)
                                    formattedAttribute +=
                                        string.Format(_localizationService.GetResource("ProductAttributes.Quantity"),
            ...
            _productAttributeParser.GetGiftCardAttribute(attributesXml, out var giftCardRecipientName,
                out var giftCardRecipientEmail, out var giftCardSenderName, out var giftCardSenderEmail, out var _);

            //sender
            var giftCardFrom = product.GiftCardType == GiftCardType.Virtual
                ? string.Format(_localizationService.GetResource("GiftCardAttribute.From.Virtual"), giftCardSenderName,
                    giftCardSenderEmail)
                : string.Format(_localizationService.GetResource("GiftCardAttribute.From.Physical"),
                    giftCardSenderName);
            //recipient
            var giftCardFor = product.GiftCardType == GiftCardType.Virtual
                ? string.Format(_localizationService.GetResource("GiftCardAttribute.For.Virtual"),
                    giftCardRecipientName, giftCardRecipientEmail)
                : string.Format(_localizationService.GetResource("GiftCardAttribute.For.Physical"),
