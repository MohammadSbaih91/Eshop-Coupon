...
namespace Nop.Web.Controllers
{
    ...
    public partial class CheckoutController : BasePublicController
    {
        ...
        protected virtual void SubscribeNewsletter(AddressModel model)
        {
            if (string.IsNullOrWhiteSpace(model.Email)) return;
            model.Email= model.Email.Trim();
            var subscription =
                _newsLetterSubscriptionService.GetNewsLetterSubscriptionByEmailAndStoreId(model.Email,
                    _storeContext.CurrentStore.Id);

            if (subscription != null)
            {
                subscription.Active = model.SubscribeNewsletter;
        ...
        public virtual IActionResult OnePageCheckout()
        {
            ...
            {
                model.BillingAddress.ExistingAddresses=model.BillingAddress.ExistingAddresses
                    .Where(s => s.StudentID != null).Where(s => s.UploadID != "").ToList();
            }
            else
            {
                model.BillingAddress.ExistingAddresses = model.BillingAddress.ExistingAddresses
                    .Where(s => s.UploadID != "").ToList();
            }
