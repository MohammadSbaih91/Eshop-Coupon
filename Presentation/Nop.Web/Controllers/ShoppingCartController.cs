...
namespace Nop.Web.Controllers
{
    public partial class ShoppingCartController : BasePublicController
    {
        ...
        protected virtual void ParseAndSaveCheckoutAttributes(List<ShoppingCartItem> cart, IFormCollection form)
        {
            ...
            _genericAttributeService.SaveAttribute(_workContext.CurrentCustomer, NopCustomerDefaults.CheckoutAttributes,
        ...
        protected virtual void AddGiftCardsAttributesXml(Product product, IFormCollection form,
                        ...
                        if (!StringValues.IsNullOrEmpty(ctrlAttributes))
                        {
                                ...
                                .Split(new[] {','}, StringSplitOptions.RemoveEmptyEntries))
                            {
                                ...
                                if (selectedAttributeId > 0)
                                {
                                    ...
                                    if (!StringValues.IsNullOrEmpty(quantityStr) &&
                                        (!int.TryParse(quantityStr, out quantity) || quantity < 1))
                                        errors.Add(_localizationService.GetResource(
                                            "ShoppingCart.QuantityShouldPositive"));

                                    attributesXml = _productAttributeParser.AddProductAttribute(attributesXml,
                                        attribute, selectedAttributeId.ToString(),
                                        quantity > 1 ? (int?) quantity : null);
                                }
                            }
                        }
                    }
        ...
        //    DateTime? rentalEndDate, int quantity, decimal subcidyDiscount)
        //{
        ...
        //            rentalStartDate, rentalEndDate, quantity, true, subcidyDiscount));
        //    }
        //    else
        //    {
        ...
        protected virtual IActionResult GetProductToCartDetails(List<string> addToCartWarnings,
        ...
        public virtual IActionResult UpdateCart(IFormCollection form)
        {
            ...
            foreach (var productId in requiredProductIds)
            {
                var item = cartItems.FirstOrDefault(c => productId == c.Product.Id)?.Id;
            ...
            var itemsWithNewQuantity = cart.Select(item => new
            {
                ...
                NewQuantity = itemIdsToRemove.Contains(item.Id) ? 0 :
                    int.TryParse(form[$"itemquantity{item.Id}"], out int quantity) ? quantity : item.Quantity,
                Item = item
        ...
        {
            var returnUrl = _genericAttributeService.GetAttribute<string>(_workContext.CurrentCustomer,
                NopCustomerDefaults.LastContinueShoppingPageAttribute, _storeContext.CurrentStore.Id);
        ...
        public virtual IActionResult StartCheckout(IFormCollection form)
        {
            ...
            var checkoutAttributeWarnings =
                _shoppingCartService.GetShoppingCartWarnings(cart, checkoutAttributes, true);
        ...
        public virtual IActionResult ApplyGiftCard(string giftcardcouponcode, IFormCollection form)
        {
            ...
            if (!_shoppingCartService.ShoppingCartIsRecurring(cart))
            {
                ...
                {
                    var giftCard = _giftCardService.GetAllGiftCards(giftCardCouponCode: giftcardcouponcode)
                        .FirstOrDefault();
                    ...
                    if (isGiftCardValid)
                    {
                        ...
                        model.GiftCardBox.Message =
                            _localizationService.GetResource("ShoppingCart.GiftCardCouponCode.Applied");
                        model.GiftCardBox.IsApplied = true;
                    }
                    else
                    {
                        model.GiftCardBox.Message =
                            _localizationService.GetResource("ShoppingCart.GiftCardCouponCode.WrongGiftCard");
            ...
            {
                model.GiftCardBox.Message =
                    _localizationService.GetResource("ShoppingCart.GiftCardCouponCode.DontWorkWithAutoshipProducts");
                model.GiftCardBox.IsApplied = false;
            }
        ...
        [PublicAntiForgery]
        [HttpPost]
        public virtual IActionResult GetEstimateShipping(int? countryId, int? stateProvinceId, string zipPostalCode,
            IFormCollection form)
        {
            var cart = _workContext.CurrentCustomer.ShoppingCartItems
                .Where(sci => sci.ShoppingCartType == ShoppingCartType.ShoppingCart)
            ...
            var model = _shoppingCartModelFactory.PrepareEstimateShippingResultModel(cart, countryId, stateProvinceId,
                zipPostalCode);
                ...
                else
                {
                    foreach (var formKey in form.Keys)
                        if (formKey.Equals($"itemquantity{sci.Id}", StringComparison.InvariantCultureIgnoreCase))
                        {
                            ...
                            {
                                var currSciWarnings = _shoppingCartService.UpdateShoppingCartItem(
                                    _workContext.CurrentCustomer,
        ...
        public virtual IActionResult EmailWishlistSend(WishlistEmailAFriendModel model, bool captchaValid)
        {
            ...
            if (_workContext.CurrentCustomer.IsGuest() && !_shoppingCartSettings.AllowAnonymousUsersToEmailWishlist)
            {
                ModelState.AddModelError("",
                    _localizationService.GetResource("Wishlist.EmailAFriend.OnlyRegisteredUsers"));
            }
