...
namespace Nop.Web.Areas.Admin.Controllers
{
    ...
    public class RoxyFilemanController : BaseAdminController
    {
        ...
        protected virtual string GetVirtualPath(string path)
        {
            ...
            {
                path = path.Replace("/app-images/uploaded", string.Empty);
            }
        ...
        protected virtual string GetFullPath(string virtualPath)
        {
            ...
            return _config.SharedFileStorageContainerName + virtualPath;
        }
        ...
        protected virtual async Task GetFilesAsync(string directoryPath, string type)
        {
            ...
            for (var i = 0; i < files.Count; i++)
            {
                ...
                var localPath = $"/app-images{directoryPath}";
                await HttpContext.Response.WriteAsync($"{{\"p\":\"{localPath.Replace("/images/", "/").TrimEnd('/')}/{_fileProvider.GetFileName(physicalPath)}\",\"t\":\"{Math.Ceiling(GetTimestamp(_fileProvider.GetLastWriteTime(physicalPath)))}\",\"s\":\"{_fileProvider.FileLength(physicalPath)}\",\"w\":\"{width}\",\"h\":\"{height}\"}}");
